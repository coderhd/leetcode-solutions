...
var insert = function (intervals, newInterval) {
    ...
    // } else {
        ...
        for (let i = 0; i < intervals.length; i++) {
            // If new interval's end is less than current interval start then return new interval and current intervals because intervals are sorted by starting point
            if (newInterval[1] < intervals[i][0]) {
                result.push(newInterval);
                const remainingIntervals = intervals.slice(i);
                return [...result, ...remainingIntervals];
            }
            ...
            console.log({ current: intervals[i], newInterval, result });
        }
